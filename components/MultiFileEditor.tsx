...
export interface MultiFileEditorHandle {
    highlightLine: (filePath: string, lineNumber: number) => void
}

const MultiFileEditor = forwardRef<MultiFileEditorHandle, MultiFileEditorProps>(
    ({
        initialFiles,
        isReadOnly = false,
        onFilesChange,
        ...
        const handleHighlightLine = useCallback((filePath: string, lineNumber: number) => {
            // Switch to the file if not already on it
            ...
            // Highlight the line
            setTimeout(() => {
                codeEditorRef.current?.highlightLine(lineNumber)
            }, 100)
        }, [activeFile])

        // Notify parent when active file changes
        useEffect(() => {
            onActiveFileChange?.(activeFile)
        }, [activeFile, onActiveFileChange])

        // Expose highlightLine method to parent via ref
        useImperativeHandle(ref, () => ({
            highlightLine: (filePath: string, lineNumber: number) => {
                if (activeFile !== filePath) {
                    setActiveFile(filePath)
                }
                setTimeout(() => {
